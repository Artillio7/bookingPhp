...
session_start();
	...
	if (isset($_POST['update'])) {
		...
		if (!empty($_POST['type']) && !empty($_POST['disponibilité']) && !empty($_POST['p_key_word']) && !empty($_POST['titre']) && !empty($_POST['description']) && !empty(($_FILES['img']['name'])) && !empty($_POST['prix']) && !empty($_POST['age'])) {
	print_r($_POST);
			//move upload img
	...
	$request->execute(array($_POST['type'], $_POST['disponibilité'], $_POST['p_key_word'], $_POST['titre'], $_POST['description'], $_FILES['img']['name'], $_POST['prix'], $_POST['age']));
		...
		<div class="log_img">
		   <br>
			...
			<div class="box1">
				...
				<form name="f" action="mise-a-jour-livres.php" method="POST" enctype="multipart/form-data">
					<div class="f">
					...
					<input type="file" name="nbjoueur" placeholder="Nombre de Joueur" required="" style="height:40px;" /><br>
					...
					<a href="mise-a-jour-jeu.php">
					<button type="submit" name="update"  onclick="verif();" style="height: 40px;border-radius: 0px;border: 0px;font-size: larger;">
						Mise a jour du jeu
					</button></a>
					</div>
				</form>
